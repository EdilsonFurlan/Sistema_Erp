...
from encaixe.models import Pedido, OrdemCompra, OrdemCompraItem, EstoqueMaterial
...
def purchase_planning(request):
    ...
    """
    # Get ID of orders that are part of an OPEN Purchase Order
    active_po_orders = Pedido.objects.filter(
        ordens_compra__status__in=['aberta', 'enviada']
    ).values_list('id', flat=True)
    
    # Get all orders
    orders = Pedido.objects.all().order_by('-data')
    
    # Annotate manually to avoid complex DB queries for now
    order_list = []
    for o in orders:
        order_list.append({
            'order': o,
            'has_open_po': (o.id in active_po_orders)
        })
        
    return render(request, 'encaixe/purchase_planning.html', {'order_list': order_list})
...
def visualize_purchase_creation(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'encaixe/purchase_creation.html', context)
...
def purchase_order_list(request):
    ...
    return render(request, 'encaixe/purchase_order_list.html', {'ocs': ocs})

def purchase_order_detail(request, oc_id):
    oc = get_object_or_404(OrdemCompra, id=oc_id)
    return render(request, 'encaixe/purchase_order_detail.html', {'oc': oc})
